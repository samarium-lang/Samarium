...
class Transpiler:
    ...
    def transpile_slice(self):
        ...
        if all(token not in tokens for token in {Token.WHILE, Token.YIELD}):
            # <<index>>
            if tokens:
                for t in tokens:
        ...
        # <<start..>>
            ...
            # <<..end**step>>
            if Token.YIELD in tokens:
                step_index = tokens.index(Token.YIELD)
        ...
        elif Token.WHILE in tokens or Token.YIELD in tokens:
            self.ch.line.append(f"{slce}(")
            ...
            step_index = index(tokens, Token.YIELD)
            # <<start..end**step>>
            if Token.WHILE in tokens and Token.YIELD in tokens:
                slices = [
