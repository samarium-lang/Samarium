...
class Pathfinder:
    def __init__(self, tokens: Iterable[int]) -> None:
        ...
        self._index = 0
        self._mark_stack: list[Waypoint] = []
    ...
    def mark(self, name: str) -> None:
        self._mark_stack.append(Waypoint(name, self._index))
        # print("MARK", self.debug(6))

    def reset(self, to: str | None = None) -> None:
        if not self._mark_stack:
            raise RuntimeError("no mark to reset to")
        idx = -1
        if to:
            while self._mark_stack[idx].name != to:
                idx -= 1
        self._index = self._mark_stack[idx].idx
        # print("RESET", self.debug(6))

    def drop(self, to: str | None = None) -> None:
        if not self._mark_stack:
            raise RuntimeError("no mark to drop")

        if not to:
            self._index = self._mark_stack.pop().idx
            return

        while (waypoint := self._mark_stack.pop()).name != to:
            pass
    ...
    def commit(self, to: str | None = None) -> None:
        if not self._mark_stack:
            raise RuntimeError("no mark to commit to")
        if not to:
            _ = self._mark_stack.pop()
            return
        while self._mark_stack.pop().name != to:
            pass
...
def watch(f: Callable[[Parser], T]) -> Callable[[Parser], T]:
    ...
    def wrapper(self: Parser) -> T:
        ...
        x = len(self._pf._mark_stack)  # pyright: ignore[reportPrivateUsage]
        # print(f"\033[32m-> (stack: {x}) {f.__name__} from {caller}\033[0m")
        # print(f"+ (stack: {x}) {f.__name__} from {caller}")
        r = f(self)
        y = len(self._pf._mark_stack)  # pyright: ignore[reportPrivateUsage]
