...
`format(string, fields)`                 | Replaces field placeholders with supplied values, e.g.:<br>`format("Hi $name!", {{"name" -> "Bob"}}) :: "Hi Bob!"`<br>`format("$$age = $age", {{"age" -> /\\}}) :: "$age = 4"`<br>When provided with an Array instead of a Table,<br>the indeces of specific values serve as keys:<br>`format("{$0@$1}", ["host", "home"]) :: "{host@home}"`
